...
const Sale = () => {
  ...
  useEffect(() => {
    fetch(`${import.meta.env.VITE_BACKEND_URL}/api/returns/sale-returns`, {
      headers: {
      ...
      >
        ...
        <div className="sm:p-4">
          <div className="flex flex-wrap gap-2 mb-4 justify-between items-center">
             ...
             <button onClick={() => navigate('/return/sale/add')} className="bg-purple-600 hover:bg-purple-700 text-white px-4 py-2 rounded-md font-medium text-sm transition-colors">
                + Create Return
             </button>
          </div>
                ...
                {returns.length > 0 ? (
                  returns.map(ret => (
                    <tr key={ret._id} className="hover:bg-gray-50">
                      ...
                      <td className="px-3 py-4">{ret.biller || 'N/A'}</td>
                      <td className="px-3 py-4">{ret.customer}</td>
